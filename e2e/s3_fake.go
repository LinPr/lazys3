...
// Package e2e contains end-to-end tests for lazys3's storage layer.
//
// The tests run against an in-process gofakes3 server by default. Set
// LAZYS3_E2E_REAL=oss (or the name of a profile in ~/.aws/config) to run
// the same suite against a real S3-compatible service such as Aliyun OSS.
// lazys3 is a TUI, not a CLI, so these tests do not shell out to a binary;
// they exercise github.com/LinPr/lazys3/internal/storage directly.
package e2e
...
// character with a dash, then trims to 42 chars and adds a 7-char random
// suffix.
