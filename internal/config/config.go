...
// $XDG_CONFIG_HOME/lazys3/config.toml (falling back to
// ~/.config/lazys3/config.toml). Every key is optional; the zero value of
...
type Theme struct {
	...
	UnfocusedBorder string `toml:"unfocused_border"`
	TitleFg         string `toml:"title_fg"`
	TitleBg         string `toml:"title_bg"`
	StatusErrorFg   string `toml:"status_error_fg"`
...
type Local struct {
	StartDir string `toml:"start_dir"` // overrides the process cwd when set
}
...
func Path() string {
	...
	return filepath.Join(base, "lazys3", "config.toml")
}

// Load reads the config from Path(). Called once at startup (cmd/root.go)
// before the TUI model is constructed.
func Load() Config {
	return LoadFrom(Path())
}

// LoadFrom reads and validates the config at path. A missing file writes
// the commented default template (best-effort) and returns defaults; a
// malformed file logs the parse error and returns defaults.
