...
func TestSelectedRowMarkAndHighlight(t *testing.T) {
	...
	m.SetSize(80, 20)
	m = load(t, m, sampleDir(t))
	m.list.Select(4) // zeta.txt
	m.ToggleSelected()
	m.list.Select(0) // move the cursor off the marked row

	d := newSelectDelegate(&m.selected)
	items := m.list.VisibleItems()
	var marked, normal strings.Builder
	d.Render(&marked, m.list, 4, items[4])
	d.Render(&normal, m.list, 3, items[3])
	...
	d.Render(&cursorMarked, m.list, 4, items[4])
