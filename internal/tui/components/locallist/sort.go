...
// sortField selects which Entry attribute orders the listing.
type sortField int
...
// sortEntries orders the slice for display: directories always come first,
...
func sortEntries(items []Entry, field sortField, desc bool) {
...
func compareBy(a, b Entry, field sortField) (less, equal bool) {
	switch field {
	case sortBySize:
...
func byName(a, b Entry, desc bool) bool {
	an, bn := strings.ToLower(a.name), strings.ToLower(b.name)
	if desc {
		return an > bn
	}
